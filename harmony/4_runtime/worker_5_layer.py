...
class Worker(object): # each rank process
    def __init__(self, args, real_dataset, shared_model, shared_optimizer, empty_model, get_lr_sched, compute_loss, save_model, XMETA, TMETA, rTASKS, CONFIGS, rank):
        ...
        # 固定内存统一管理的逻辑现在这写一下
        # （1）获取当前rank上的所有层
        # 需要注意的是，不能只拿BWD的vt的layer，因为前后向vt执行层不一样（根据实际情况看通常只有一小部分重合）
        ...
        # 📍专门提供给partition_param_swapper，用于识别要拿的layer在哪个rank文件夹
        # 📌对harmony DP来说，全都被映射到最后一个rank对应的文件夹了
        layer_id_to_rank = {}
        ...
        # 📍这里只创建学习率调度器即可，layer的pinned 版本后面统一创建，因为现在即使是当前rank负责的layer也不用
        #    全部创建了，创建最大的那个vt的layer数量即可
        # 24/12/27：只为cpu layer创建pinned memory，即除transformer layer以外的层
        ...
        for id, optim in enumerate(shared_optimizer):
            ...
            # 📍只有预先设置的在cpu上的层才会初始化pinned版本，不然pinned model就是None
        ...
        # 已经确定Loss计算层调用parameters不会返回任何东西，也不会报错。
        # print(f"rank:{self.rank},打印Loss计算层的parameters()：{shared_model[27][0].parameters()}")
        # length = len(list(shared_model[27][0].parameters()))
        # print(f"rank:{self.rank}，Loss计算曾parameters迭代起的长度为:{length}")
        # for param in shared_model[27][0].parameters():
        #     print(f"rank:{self.rank}, param")
        # exit(0)
        # 
        # TODO: 包含首层和最后一层的vt，要注意映射的问题
        ...
        # import sys
        # sys.path.append('/home/dingyuquan/project/gitcode/harmony')
        ...
        # 记录一个transformer层每个param idx到其shape的映射
        ...
        # 🔛: 低一个为子层粒度的卸载提供；第二个专门为层粒度的卸载提供
        # self.pinned_buffer_information = shared_optim_cpu.PinnedBufferInformation(max_trans_layers, shared_model[1][0], self.rank)
        self.pinned_buffer_information = shared_optim_cpu.PinnedBufferInformation_2(max_trans_layers, shared_model[1][0], self.rank)

        # 一个transformer layer的参数量和补齐后的参数量
        ...
        # UpdateInBkgd_for_worker5 所需，进行 share model->nvme 
        # 🔛
        # self.swap_to_nvme_handler = shared_optim_cpu.SwapToNVMeInBkgd(self.shared_model_nvme_handler, self.layer_id_to_layer_idx, bwd_tasks, layer_num, self.rank)
        ...
        # 🔛
        # syncpin_handler = shared_optim_cpu.SyncPinModelInBkgd_2(shared_optimizer, self.rank, nvprof=self.nvprof)
        ...
        # 🔛
        # PrefetchLocalModelGPU_for_worker5 所需，进行nvme->cpu pinned buffer
        # self.swap_in_cpu_handler = shared_optim_cpu.SwapInCpuInBkgd(syncpin_handler, self.shared_model_nvme_handler, self.layer_id_to_layer_idx, layer_num, self.rank)

        
        # 废弃，以迁移至该类内部执行
        # 将当前rank上所有BWDvt的layer的shared model清空(并非delete)，并卸载到nvme
        # 各个rank上的BWD layer是独有的，加在一起就是一个完整的model
        # self.swap_to_nvme_handler.swap_to_nvme_at_start(bwd_tasks)
        self.pcm.print("rank%d: -----------------------删除shared model参数之前" % self.rank)
        self.shared_model_nvme_handler.swap_out_from_shared_memory_through_pinned_buffer(bwd_tasks)
        self.pcm.print("rank%d: -----------------------删除shared model参数之后" % self.rank)

        # TODO: 不删除剩余的层，这些层会指向shared model，导致shared model无法被删除
        # DONE
        self.shared_model_nvme_handler.delete_other_shared_model(other_layers)
        print(f"rank:{self.rank}, {other_layers}")
        self.pcm.print("rank%d: -----------------------删除其他shared model参数之后" % self.rank)


        gc.collect()
        self.pcm.print("rank%d: -----------------------最终" % self.rank)
        # exit(0)
        # for id, optim in enumerate(shared_optimizer):
        #     if optim.shared_optimizer is not None:
        #         print(f"rank:{self.rank}, optimizer:{id}")
        #         for param, state in optim.shared_optimizer.state.items():
        #             # state是一个字典，包含该参数的所有优化器状态
        #             for k, v in state.items():
        #                 if isinstance(v, torch.Tensor):
        #                     print(f"rank:{self.rank}, param shape:{param.shape}, state:{k}, is_shared:{v.is_shared()}")
        # exit(0)
        ###################################################################################################
        ...
        # 🔛
        # self.update_handler = shared_optim_cpu.UpdateInBkgd_for_worker5(self.default_stream, shared_optimizer, self.swap_to_nvme_handler, lr_scheduler, self.rank, nvprof=self.nvprof)
        ...
        # 🔛
        # self.prefetch_model_handler = local_model_gpu.PrefetchLocalModelGPU_for_worker5(syncpin_handler, self.swap_in_cpu_handler, self.local_model, layer_num, self.rank, swapin_stream=None, compute_stream=self.default_stream, nvprof=self.nvprof)
    ...
    def _a_pack_forward_an_ubatch(self, vt, ubatch_idx, ubatch_size,
        ...
        if not vt.has_criterion: # not last pack yet
            ...
            if not requires_grad:
                ...
                # print(f"rank:{self.rank}, 激活的参数量:{num_elements}, 激活的数据类型:{Y_tensors[0].dtype}, 激活的内存占用: {memory_usage} mb",flush=True)
        ...
        # FWD任务, 要先执行FWD产生一个loss
            ...
            # print(f"rank:{self.rank}, vt{vt.idx}({vt.type})({vt.show_layers()})({ubatch_idx})准备接收时间:{now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}", flush=True)
            ...
            now = datetime.datetime.now()
            # print(f"rank:{self.rank}, vt{vt.idx}({vt.type})({vt.show_layers()})({ubatch_idx})接收完成时间:{now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}", flush=True)
            ...
            # print(f"rank:{self.rank}, vt{vt.idx}({vt.type})({vt.show_layers()})({ubatch_idx})recompute完成", flush=True)
            ### In {T}
    ...
    def _a_pack_backward_an_ubatch(self, vt, ubatch_idx, ubatch_size,
        ...
        # 1.2.非第一个BWD任务，须接收前一个BWD任务发来的dY
            ...
            if m.medium == "P2P":
                ...
                if self.args.no_p2p_prerecv:
                    ...
                    # now = datetime.datetime.now()
                    # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx})接收时间:{now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}", flush=True)
        ...
        # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx}) 从接收语句中退出", flush=True)
            ...
            if isinstance(Y,(torch.Tensor, Variable)) and (Y.requires_grad):
                ...
                for i, y in enumerate(Y):
                    if isinstance(y,(torch.Tensor, Variable)) and (y.requires_grad):
                        Y_tensors.append(y)
                        Y_gradients.append(dY_named_tensors[name][i])
        # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx})准备进行反向传播", flush=True)
        ...
        # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx})反向传播执行完成", flush=True)
        ...
        if vt.Out['dX']:
            ...
            if m.medium == "P2P":
                ...
                # now = datetime.datetime.now()
                # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx})发送时间:{now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}", flush=True)
        ...
        del X_named_tensors; del Y_named_tensors
        del dY_named_tensors; del Y_tensors; del Y_gradients
        # print(f"rank:{self.rank}, vt{vt.idx}({vt.show_layers()})({ubatch_idx})del执行完成", flush=True)
    ...
    def run_training_loop(self):
        ...
        for epoch in range(self.args.num_epochs): # traverse epoches
            for it, minibatch in enumerate(self.data_loader): # traverse each minibatch
                ...
                for j, vt in enumerate(self.rTASKS[self.rank]): # { rank0: [task0,task2,...] }
                    ...
                    if vt.type == 'FWD' and vt.is_gpu:
                        ...
                        with torch.no_grad():
                            ...
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, 取模型执行了:{execution_time:.6f}秒",flush=True)
                            ...
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, microbatch{i}, 删除执行了:{execution_time:.6f}秒",flush=True)
                        ...
                        end_time = time.time()
                        execution_time = end_time - start_time
                        print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, 取模型执行了:{execution_time:.6f}秒", flush=True)
                        get_time.append(execution_time)
                        ...
                        for i, u in enumerate(vt.ubatchszs):
                            ...
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, microbatch{i}, recompute执行完成", flush=True)
                            ...
                            self._a_pack_backward_an_ubatch(vt, i, u,
                                                        ...
                                                        sucinfo=self.sucinfo,
                                                        iteration_num=it)
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, microbatch{i}, BWD执行完成", flush=True)
                            end_time = time.time()
                            execution_time = end_time - start_time
                            ...
                            del X_named_tensors; del Y_named_tensors # very important!
                            start_time = time.time()
                            # memory_monitor.mark_event(f"Before gc.collect in microbatch{i} - Iteration {it}")
                            gc.collect()
                            # memory_monitor.mark_event(f"no gc.collect in microbatch{i} - Iteration {it}")
                            end_time = time.time()
                            execution_time = end_time - start_time
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, microbatch{i}, gc.collect执行了:{execution_time:.6f}秒")
                            gc_time.append(execution_time)
                        ...
                        if self.CONFIGS["opt_offld"]:                   
                            ...
                            ### Out {W,dW,B}
                            # 阻塞，直到该流中的任务全部完成
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, BWD 准备synchronize", flush=True)
                            self.default_stream.synchronize() # CPU wait
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, BWD synchronize完成", flush=True)
                            ...
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, 卸载执行了:{execution_time:.6f}秒", flush=True)
                            ...
                            execution_time = end_time - start_time
                            delete_time.append(execution_time)
                            print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, 删除执行了:{execution_time:.6f}秒", flush=True)

                            start_time = time.time()
                            # memory_monitor.mark_event(f"Before gc.collect - Iteration {it}")
                            gc.collect()
                            # memory_monitor.mark_event(f"no gc.collect in microbatch{i} - Iteration {it}")
                            end_time = time.time()
                            execution_time = end_time - start_time
                        ...
                        if self.CONFIGS["mode"]=='vPP' or (self.CONFIGS["mode"]=='vDP' and self.rank==0):
                            if not self.args.no_update:
                                ...
                                print(f"rank{self.rank}, vt.idx:{vt.idx}, {vt.type}, {vt.layers}, 已从update_handler get_queue中拿到")
                                assert vt_idx == vt.idx
                                update_time.append(step_time)
                                offload_time.append(off_time)
                        if self.nvprof: nvtx_range_pop() 
                        # -----------------------------------------------
                    else:
                        raise ValueError("Unknown vTask.type {} with .device {} !".format(vt.type,vt.device))
                ...
                print(f"rank{self.rank}, 当前minibatch的样本数:{self.CONFIGS['D']}, 每秒处理样本数:{samples_per_sec}")
                ...
                self.offload_time.append(sum(offload_time))
                self.update_time.append(sum(update_time))

                print(f"rank{self.rank}, 取模型共花费:{self.get_time[-1]}'s, 卸载至nvme花费:{self.offload_time[-1]}'s, 计算共花费:{self.compute_time[-1]}'s, 删除模型共花费:{self.delete_time[-1]}'s, 卸载梯度buffer共花费:{self.swapout_time[-1]}'s, 卸载至nvme花费:{self.offload_time[-1]}'s, gc.collect共花费:{self.gc_time[-1]}'s")
                if self.args.no_update:
                ...
                self.pcm.print("rank%d: 第%d次迭代后" % (self.rank, it))
        ### end training
        torch.cuda.synchronize(self.rank)
        dist.barrier()
    ...
    def finish(self): 
        ...
        avg_offload_time = np.mean(self.offload_time[self.avg_it:])
        avg_update_time = np.mean(self.update_time[self.avg_it:])
        ...
        if self.rank == 0:
            ...
            print(f"[Global] 平均计算时间:{avg_compute_time}, 平均删除时间:{avg_delete_time}, 平均垃圾回收时间:{avg_collect_time}, 平均拿取时间:{avg_get_time}, 平均卸载梯度buffer时间:{avg_swapout_time}, 平均卸载至nvme时间:{avg_offload_time}, 平均更新时间:{avg_update_time}")
            print(f"[Global] nvme时间占总体时间的比例:{(avg_get_time + avg_offload_time) / avg_iter_time}")
