...
def worker_func(*pargs, **kwargs): # per process
    # 📍切换版本
    # worker_5_for_cpu：以transformer layer内部的param的粒度
    #                   偶尔会出现假死的问题。经过修改以从频繁假死变为偶尔假死
    # worker_5_layer: 以整个transformer layer为粒度卸载
    # worker_5_layer_2_shared: 以transformer layer为粒度 + 双区域shared memory
    # worker_5_double_buffer: 以transformer layer为粒度 + 双pinned buffer
    from worker_5_layer import Worker
...
def run(args, real_dataset, create_model, create_optimizer, get_train_steps=None, get_lr_sched=None, compute_loss=None, save_model=None): # main process
    ...
    # print(optimizer)
    # 可以确定，最后一层，即计算损失的层的优化器是None
    pcm.print("optimizer created")
    
    # initialize shared optimizer on CPU
    from shared_optim_cpu import SharedOptimCPU_for_worker5
    ...
    for id, ((vlayer, _, _), optim) in enumerate(zip(shared_model, optimizer)): 
        ...
        shared_optimizer.append(SharedOptimCPU_for_worker5(vlayer, optim, id))
